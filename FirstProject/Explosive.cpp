...
{
	Super::OnOverlapBegin(OverlappedComponent, OtherActor, OtherComp, OtherBodyIndex,
		...
		if (Main)
		{
			if (OverlapParticles)
			{
				UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), OverlapParticles,
					GetActorLocation(), FRotator(0.f), FVector(1.f), true);
			}
			if (OverlapSound)
			{
			...
			if (DamageTypeClass)
			{
				UGameplayStatics::ApplyDamage(Main, Damage, BombInstigator, this,
					DamageTypeClass);
			}
			Destroy();
		}
