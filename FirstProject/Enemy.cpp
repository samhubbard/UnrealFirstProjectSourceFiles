...
void AEnemy::AggroSphereOnOverlapBegin(UPrimitiveComponent* OverlappedComponent,
...
void AEnemy::AggroSphereOnOverlapEnd(UPrimitiveComponent* OverlappedComponent,
	AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	if (OtherActor)
	{
	...
	bool bFromSweep, const FHitResult& SweepResult)
{
	if (OtherActor)
	{
		...
		if (Main)
		{
			...
			if (DamageTypeClass)
			{
				UGameplayStatics::ApplyDamage(Main, Damage, AIController,
